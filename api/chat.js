...
function requestContextOidcToken() {
  ...
  return clean(context.headers?.['x-vercel-oidc-token'], 20_000)
}

function gatewayToken() {
  // Production Vercel Functions expose the short-lived project OIDC token through
  // request context. Environment-based values remain supported for local/build use.
  return clean(process.env.AI_GATEWAY_API_KEY, 20_000)
    || requestContextOidcToken()
